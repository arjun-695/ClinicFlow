...
export async function main(configName: string): Promise<void> {
  ...
  globalLogger.info('Loaded Redis Config', { redis: config.redis });
...
if (import.meta.main || (process.argv[1] && (process.argv[1].endsWith('index.ts') || process.argv[1].endsWith('index.js')))) {
